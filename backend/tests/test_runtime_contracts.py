...
def test_backend_pyproject_declares_runtime_and_test_dependencies_for_uv(
    repo_root: Path,
) -> None:
    pyproject_path = repo_root / "backend" / "pyproject.toml"

    assert pyproject_path.is_file(), (
        "backend/pyproject.toml must exist for uv-managed backend dependencies"
    )
    ...
    test_group_names = [
        group_name for group_name in ("dev", "test") if group_name in dependency_groups
    ]
    test_dependencies = [
        dependency
...
def test_backend_uv_lockfile_is_committed_for_the_uv_managed_project(
    repo_root: Path,
) -> None:
    lockfile_path = repo_root / "backend" / "uv.lock"

    assert lockfile_path.is_file(), (
        "backend/uv.lock must be committed for clean-checkout uv installs"
    )
    ...
    assert tracked_lockfile.returncode == 0, (
        "backend/uv.lock must be tracked in git, not left as an uncommitted local artifact"
    )


def test_backend_uv_managed_project_removes_legacy_requirements_txt_contracts(
    repo_root: Path,
) -> None:
    legacy_requirements = [
    ...
    assert legacy_contracts_still_present == [], (
...
def test_vite_and_vitest_configs_use_module_filenames_under_a_commonjs_package_boundary(
    ...
    assert package_json.get("type") == "commonjs", (
        "this contract applies only while the repo package boundary remains CommonJS"
    )

    expected_module_configs = [
        repo_root / "vitest.config.mts",
        repo_root / "frontend" / "vite.config.mts",
    ]
    legacy_ts_configs = [
    ...
    assert missing_module_configs == [], (
        "expected module-signaling config filenames are missing"
    )
    assert legacy_configs_still_present == [], (
