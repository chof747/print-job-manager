def test_root_backend_dev_command_declares_a_static_uvicorn_entrypoint(
    package_json: dict,
) -> None:
    ...
    assert "uvicorn" in script, (
        "dev:backend should invoke the FastAPI backend through uvicorn"
    )
...
def test_root_backend_dev_command_runs_uvicorn_through_uv_from_the_repo_root(
    package_json: dict,
) -> None:
    script = package_json["scripts"].get("dev:backend")
    ...
    assert "uv run" in script, (
        "dev:backend should execute uvicorn through uv so the backend pyproject contract owns dependency resolution"
    )
    assert "backend" in script, (
        "dev:backend should explicitly target the backend project when launched from the repo root"
    )
...
def test_root_backend_check_command_declares_a_static_pytest_flow(
    package_json: dict,
) -> None:
    ...
    assert "pytest" in script, (
        "check:backend should run pytest through the root command"
    )
    assert "pip install" not in script, (
...
def test_root_backend_check_command_runs_pytest_through_uv_from_the_repo_root(
    package_json: dict,
) -> None:
    script = package_json["scripts"].get("check:backend")
    ...
    assert script, "package.json must define a check:backend root convenience command"
    assert "uv run" in script, (
        "check:backend should execute pytest through uv so the backend pyproject contract owns dependency resolution"
    )
    assert "backend" in script, (
        "check:backend should explicitly target the backend project when launched from the repo root"
    )
...
def test_root_frontend_dev_command_declares_a_static_vite_flow(
    package_json: dict,
) -> None:
    ...
    assert "frontend" in script, (
        "dev:frontend should explicitly target the frontend app from the repo root"
    )
    assert "frontend/vite.config.mts" in script, (
    ...
    assert "vite" in dependencies or "vite" in dev_dependencies, (
...
def test_root_frontend_check_command_declares_an_explicit_frontend_vitest_flow(
    ...
    assert "vitest" in script, (
        "check:frontend should run Vitest through the root command"
    )
    assert "npm install" not in script, (
